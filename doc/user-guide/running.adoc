...
`default_values.conf.sample` file. This file (with the .sample ending removed) can be used to specify the default values to be inserted when a field was omitted from the C-DNS output. In the sample all the fields are commented out so no defaults would be used. The fields are specified in logical groups similar to <<excludes>>. The file looks like this
...
The comments on each line show the format of allowed values and in some cases the set of allowed values. For example to set a default for qr-transport-flags you could edit the file to say:
----
