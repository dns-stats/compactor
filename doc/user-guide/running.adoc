...
Options:
  ...
  -z [ --gzip-output ]         compress output data using gzip. Adds .gz 
                               extension to output file.
  -y [ --gzip-level ] arg (=6) gzip compression level.
  -x [ --xz-output ]           compress output data using xz. Adds .xz extension 
                               to output file.
  ...
  -I [ --info-only ]           don't generate output files, only info files.
...
Briefly, this means that client and server IP addresses in PCAP output are
pseudo-anonymised, as are all IP addresses in counts of events recorded. IP
addresses in data returned from server to client are not anonymised. Client IP
