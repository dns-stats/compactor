...
Options:
  -h [ --help ]                         show this help message.
  -v [ --version ]                      show version information.
  --defaultsfile arg (=/usr/local/etc/dns-stats-compactor/default_values.conf)
                                        default values file.
...
Most fields in C-DNS are optional. See the <<excludes>> and the deprecated <<options>> setting for _compactor_. When converting D-DNS to PCAPs or templated text output it may be necessary to fill in those omitted fields with default values.

In order for the inspector to be able to generate sane PCAP files from C-DNS with any set of excluded fields a default values file must be present and must contain the full set of default values. If this file is not present or doesn't contain the required default values _inspector_ will exit with an error.

==== Default values file location

On startup, _inspector_ looks for a default values file. By default this is named default_values.conf and is located in a `dns-stats-compactor` system configuration directory. A different location can specified on the command
...
qr-transport-flags=ipv4 udp     # (ipv4|ipv6) (udp|tcp) (trailing-data) (TODO: REMOVE??)
...
NOTE: `dns-flags` is the only default that can have no value specified in order to set all flags to 0. It is also the only default that can have multiple values.

==== Combining excluded fields, default values and pcap-filters

The resulting PCAPs produced by excluding response fields:
----
response-delay
response-rcode
...
will be different to using a pcap filter
----
filter=dst host <serverIP>
----
This is because _compactor_ records that it saw a response in the first case, even if it doesn't record anything about the response. In the latter case _compactor_ never saw the response.
