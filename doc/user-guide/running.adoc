...
`/etc/dns-stats-compactor/default_values.conf.sample` 

==== Default configuration

Both installing via binary packages and installing using the source distribution will install a sample
`default_values.conf.sample` file. This file (with the .sample ending removed) can be used to specify the default values to be inserted when a field was omitted from the C-DNS output. In the sample all the fields are commented out so no defaults would be used. The fields are specified in logical groups similar to the excludes_fields file <<excludes>>. The file looks like this
...
The comments on each line show the format of allowed values and in some case the set of allowed values. For example to set a default for qr-transport-flags you could edit the file to say:
----
qr-transport-flags=ipv4 udp
----

Some of these defaults are ignored if you are generating PCAP files. For example, query-ancount is ignored and the actual count of rr's in the answer section of the DNS message is used. This ensures that valid DNS packets are generated. Other values like qr-type have no meaning in a PCAP file.
