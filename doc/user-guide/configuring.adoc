...
If the excluded fields file exists and there are *include* lines in the configuration file an error will be
logged and the _compactor_ will exit. 

If the excluded fields file exists then the _compactor_ will act as if *include=all* was present. The excluded fields
file then controls what fields are *excluded* from the C-DNS output.

The excluded fields are stored in the C-DNS file as hints for the _inspector_ to use when processing C-DNS files.
...
which fields to be excluded from the C-DNS output. In the sample all the fields are commented out so all 
